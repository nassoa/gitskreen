...
import { AlertCircle, GitCompare, FileCode, GitCommit, Plus, Minus, FileText } from "lucide-react"
...
export default function BranchComparison({ repoData }) {
  ...
  useEffect(() => {
    ...
    if (repoData?.branches?.length > 0) {
      ...
      const otherBranch = repoData.branches.find((branch) => branch.name !== defaultBranch)
      if (otherBranch) {
  ...
  const compareRepositories = async () => {
    if (baseBranch === compareBranch) {
      setError("Veuillez sélectionner deux branches différentes pour la comparaison.")
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erreur lors de la comparaison des branches: ${response.status}`)
      }
      ...
      setError(error.message || "Une erreur s'est produite lors de la comparaison des branches.")
    } finally {
      setLoading(false)
    }
  }
  ...
  const renderDiffContent = (patch) => {
    ...
    return lines.map((line, index) => {
      ...
      return (
        <div key={index} className={`font-mono text-xs whitespace-pre ${getLineClass(type)} px-2 py-0.5`}>
          {line}
        </div>
      )
    })
    ...
    <div className="grid gap-6 mt-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Comparez les différences entre deux branches du dépôt</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              <label className="text-sm font-medium mb-2 block">Branche de base</label>
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Branche à comparer</label>
              <Select value={compareBranch} onValueChange={setCompareBranch}>
                ...
                <SelectContent>
                  {repoData?.branches?.map((branch) => (
                    <SelectItem key={`compare-${branch.name}`} value={branch.name}>
                      {branch.name}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <Button onClick={compareRepositories} disabled={loading || !baseBranch || !compareBranch}>
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Comparaison de <strong>{baseBranch}</strong> avec <strong>{compareBranch}</strong>
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-muted rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold">{comparisonData.total_commits}</div>
                  <div className="text-sm text-muted-foreground">Commits</div>
                </div>

                <div className="bg-muted rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold">{comparisonData.files?.length || 0}</div>
                  <div className="text-sm text-muted-foreground">Fichiers modifiés</div>
                </div>
              ...
              <div className="mb-6">
                ...
                <Badge variant={comparisonData.status === "ahead" ? "default" : "secondary"} className="mb-2">
                ...
                <p className="text-sm text-muted-foreground">
                  {comparisonData.status === "identical"
                    ...
                    : `Comparaison entre ${comparisonData.merge_base_commit?.sha.substring(0, 7)} et ${
                        comparisonData.commits[comparisonData.commits.length - 1]?.sha.substring(0, 7) || ""
                      }`}
                </p>
              </div>
              ...
              {comparisonData.commits && comparisonData.commits.length > 0 && (
                <div>
                  <h3 className="text-lg font-medium mb-2">Commits ({comparisonData.commits.length})</h3>
                  <div className="border rounded-md overflow-hidden">
                    <ScrollArea className="h-[300px]">
                      {comparisonData.commits.map((commit, index) => (
                        <div key={commit.sha} className="p-3 border-b last:border-0">
          ...
          {comparisonData.files && comparisonData.files.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Fichiers modifiés ({comparisonData.files.length})</CardTitle>
                <CardDescription>Détails des modifications de fichiers entre les branches</CardDescription>
              </CardHeader>
              <CardContent>
                <Tabs defaultValue="files-list">
                  <TabsList className="mb-4">
                    <TabsTrigger value="files-list">Liste des fichiers</TabsTrigger>
                    ...
                    <div className="border rounded-md overflow-hidden">
                      <ScrollArea className="h-[400px]">
                        {comparisonData.files.map((file, index) => (
                          <div key={file.filename} className="p-3 border-b last:border-0">
                            <div className="flex items-start justify-between">
                              ...
                              <Badge variant="outline">
                                {file.changes} changement{file.changes > 1 ? "s" : ""}
                              </Badge>
                            </div>
                          </div>
                        ))}
                      ...
                      {comparisonData.files.map((file) => (
                        <div key={file.filename} className="border rounded-md overflow-hidden">
                          ...
                          <ScrollArea className="max-h-[300px]">
                            <div className="p-1">{renderDiffContent(file.patch)}</div>
                          </ScrollArea>
                        </div>
                      ))}
                    </div>
